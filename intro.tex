...
Haṭhayoga taucht ausnahmslos in allen komplexen spätmittelalterlichen Yogataxonomien auf. In den Taxonomien mit fünfzehn Yogas der \textit{Yogatattvabindu}, dem \textit{Yogasvarodaya} und der \textit{Yogasiddhāntacandrikā} nimmt es jeweils die vierte Position ein. In der zwölffachen Taxonomie von Sundardās \textit{Sarvāṅgayogapradīpikā} steht es an der fünften Position und ist somit neben einer eigenen Kategorie gleichzeitig auch die Oberkategorie für die drei nächsten Yogas die von Sundardās beschrieben werden, nämlich Rāja-, Lakṣa- und Aṣṭāṅgayoga.

\subsection{Haṭhayoga in the \textit{Yogatattvabindu} and \textit{Yogasvarodaya}}

Im \textit{Yogatattvabindu} in Sektion \uproman{19}-\uproman{20} werden zwei Unterscheidungen des Haṭhyoga beschrieben. Sie basieren auf den Ausführungen des \textit{Yogasvarodaya},\footnote{Siehe\textit{ramatosana}, Ed. p. 835 und \citetitle{shabdakalpadruma}, Ed. p. 501. Diese Passagen beinhalten Zitate des \textit{Yogasvarodaya} von beiden Arten des Haṭhayoga. Siehe auch \citetitle{yogakarnika} 12.23-26. Hier werden Verse der zweiten Kategorie des Haṭhayoga wiedergegeben.} und weichen hinsichtlich der Formulierungen nur leicht leicht von diesen ab. Beide Passagen bestechen durch ihre Kürze. 

Der erste in beiden Texten beschriebene Typus des Haṭhayoga inkludiert die Kontrolle des Atems durch Ausatmen (\textit{recaka}), Einatmen (\textit{pūraka}) und Atmenanhalten (\textit{kumbhaka}) etc. Mit Begriff ``etc.'' (\textit{°ādi°}) verweist der Text wahrscheinlich auf andere bekannte Praktiken des \textit{Haṭhayoga}. Hiermit könnten neben anderen Atemübungen auch die anderen bekannten Grundbausteine des Haṭhayoga gemeint sein, die seit Svātmarāmas \textit{Haṭhapradīpikā} populär waren: \textit{āsana}, \textit{mudrā} und \textit{nādānusandhāna}. Zumindest \textit{āsana} wird auch in der \textit{Yogasvarodaya} explizit genannt (\textit{kṛtvāsanaṃ pavanāśaṃ śarīre rogahārakam}). Im Anschluss nennen beide Texte die den Körper reinigenden sechs Aktionen (\textit{ṣatkarma}). Dann verlautbart Rāmacandra, dass wenn der volle Atem im innern des Sonnenkanals (\textit{sūryanāḍi}) verweilt, der Geist unbeweglich wird. Durch die Bewegungslosigkeit des Geistes entsteht Glückseeligkeit und der Geist wird in die Leere (\textit{śūnya}) absorbiert. Dies führt zu Unsterblichkeit (\textit{kālaḥ samīpe nāgachati}). Die Nennnung des Sonnenkanals ist in diesem Kontext aufällig. Das \textit{Yogasvarodaya} ist hier keine konkrete Hilfe, da sie lediglich von einem nicht näher spezifizierten \textit{nāḍī} spricht,\footnote{Da im YSv kein spezifischer \textit{nāḍī} genannt ist, sollte zumindest in diesem Fall unbedingt davon ausgegangen werden, dass es sich um den \textit{nāḍī} \textit{par excellance}, die \textit{suṣūmnā} handelt.} in welchem ausgelöst durch die vorangehende Praxis die Fülle des Atems verharrt (\textit{etan nāḍyān tu deveśi vāyupūrṇaṃ pratiṣṭhitam} | \textit{tato mano niścalaṃ syāt tata ānanda eva hi} |). Die Mehrheit der Texte des Haṭhayoga Genre würde vor dem Hintergrund der ``Bewegungslosigkeit des Geistes'', zentrales Characteristikum des \textit{samādhi}-Zustandens gewiss \textit{suṣūmnā}, den zentralen Kanal nennen, und nicht den mit der Sonne assoziierten rechten Kanal namens Piṅgalā, da der Eintritt des Yogazustandes, bzw. \textit{samādhi} oftmals mit dem Eintritt des Atems in den zentralen Kanals assoziert wird.\footnote{Dies ist z.B. bereits im ältesten schriftlichen Zeugnis des Haṭhyoga Genre, der \textit{Amṛtasiddhi} 26.1-2 (\textit{yo 'sau siddhimayo vāyur madhyamāpadaniścalaḥ} | \textit{tadānandamayaṃ cittam ekarūpaṃ nabhaḥsamam} || 26.1 || \textit{yadānandamayaṃ cittaṃ bāhyakleśāvivarjitam} | \textit{bhavaduḥkhāni saṃhṛtya samādhir jāyate tadā} || 26.2 ||) der Fall. \citeauthor{asiddhi} übersetzt: (1) `When Breath is perfected and fixed in the place of the Goddess of the Centre, then consciousness has the nature of bliss, uniform like the sky.' (2) `When consciopusness has the nature of bliss, free from external afflictions, then, having the sorrows of existence, Samādhi arises.' Diese in diesem Genre bereits ab spätestens dem 11. Jh. anzutreffende Idee durchdringt in der Folge des gesamte Genre.} Entweder ist der Begriff \textit{sūryanāḍi} hier als unglücklich\footnote{Im Sinne der Zweideutigkeit und Überlappung mit dem \textit{piṅgalā}-Kanals.} gewähltes Synonym zu verstehen, oder der Text ist korrupt.\footnote{Eine Konjektur von \textit{sūryanāḍī} zu \textit{śūnyanāḍī} wäre naheliegend. In \textit{Jyotsnā} 4.10, versteht Brahmānanda ``die Leere'' (\textit{śūnya}) als den zentralen Kanal. In \textit{Haṭhapradīpikā} 3.4, ist \textit{śūnyapādavī} ein Synonym von \textit{suṣumnā}.} Eine letzte Möglichkeit wäre es, tatsächlich eine mit dem \textit{piṅgalā}-Kanal assoziierte Praxis anzunehmen. Tatsächlich findet sich die Bezeichnung \textit{sūryanāḍī} in der \textit{Siddhasiddhāntapaddhati}, einem Text der Rāmacandra ebenfalls als Vorlage diente.\footnote{Cf. \textit{Siddhasiddhāntapaddhati} 2.5 (\textit{pañcamaṃ kaṇṭhacakraṃ caturaṅgulaṃ tatra vāme iḍā candranāḍī dakṣiṇe piṅgalā sūryanāḍī tanmadhye suṣumnāṃ dhyāyet saivānāhatakalā anāhatasiddhir bhavati} |}

Der zweite Typus des Haṭhayoga im \textit{Yogatattvabindu} instruiert eine unspezifische Form (\textit{kiṃcidrūpā}), in den Farben weiß, gelb, blau und rot gleich den Glanz von zehn Millionen Sonnen im eigenen Körper von Kopf bis Fuß zu kontemplieren (\textit{cintyate}). Dies soll alle Krankheiten des Körper wegbrennen und das Leben verlängern. In der \textit{Yogasvarodaya} ist nicht von einer unspezifischen Form die Rede, sondern diese Farben und der Sonnenglanz soll im Bereich der Nasenspitze kontempliert werden (\textit{ākāśe nāsikāgre tu sūryakoṭisamaṃ smaret} | \textit{śvetaṃ raktaṃ tathā pītaṃ kṛṣṇam ity ādirūpataḥ} |).

\subsection{Haṭhayoga in the \textit{Yogasiddhāntacandrikā}}

In der \textit{Yogasiddhāntacandrikā} verteilt sich die Diskussion und Beschreibung von Nārāyaṇatīrthas Haṭhayoga über mehrere \textit{sūtra}s der ersten beiden Kapitel, dem \textit{samādhipāda}s (1.34) und dem \textit{sādhanapāda}s (2.46-52). Der Kommentar von Nārāyaṇatīrtha ist hier besonders ausführlich und detailliert.

Nārāyaṇatīrtha verortet Haṭhayoga\footnote{For an earlier, short discussion of Haṭhyoga in Nārāyaṇatīrtha's \textit{yogacandrika} see \citeauthor{penna2004}, 2004: 76.} zunächst im Kontext von \textit{sūtra} 1.34. Dieses \textit{sūtra} ist eine unter verschiedenen Optionen (1.32-40), die angewandt werden können, um die in den \textit{sūtra}s 1.30-31 beschriebenen Ablenkungen des Geistes (\textit{cittavikṣepāḥ}), welche von dem im \textit{Pātañjalayogaśāstra} angestrebten Yogazustand (\textit{asaṃprajnātasamādhi} bzw .\textit{kaivalya}) ablenken, zu erreichen:
...
Die in 1.34 genannte Methode dient der Etabliereung eines klaren Geistes und wird von Nārāyaṇatīrtha als Haṭhayoga bezeichnet. In seinem Kommentar erklärt Nārāyaṇatīrtha, dass der Begriff \textit{pracchardana} das langsame nach außen Entleeren der Atemluft des Bauches durch eines der beiden Nasenlöcher in gemessenen Mengen bedeutet.\footnote{\citetitle{yogacandrika} 1.34 (Ed. p. 53): \textit{kauṣṭhyasya vāyoḥ pracchardanam, ekataranāsāpuṭena mātrāpramāṇena śanaiḥ śanair bāhar niḥsāraṇam |}} Der Begriff \textit{vidhārana} ist die äußere andauernde Atemanhaltung der ausgeatmenten Luft.\footnote{\citetitle{yogacandrika} 1.34 (Ed. p. 53): \textit{vidhāraṇaṃ recitasya vāyor bahir eva sthāpanaṃ kumbhakaṃ |}} Desweiteren nennt Nārāyaṇatīrtha diese Methode der Atemhaltung \textit{recitakumbhaka}. Sie gilt als erste unter insgesamt sieben Atemanhaltungen (\textit{saptakumbhaka}) als besonders preisenswerth, da für diesen Typus der Atemanhaltung auf keine vielen Regeln Rücksicht genommen werden muss. Die insgesamt sieben \textit{kumbhaka}s - \textit{recita, pūrita, śānta, pratyāhāra, uttara, ādhāra} und \textit{sama} - werden aber erst im zweiten Kaptiel im Kontext des vierten Gliedes des \textit{aṣṭāṅgayoga} namens \textit{prāṇāyāma} (2.49-53) zusammen mit weiteren \textit{kumbhaka}s ausführlicher behandelt.\footnote{\citetitle{yogacandrika} 1.34 (Ed. p. 53): \textit{tathā cātra pūrakavarjanād recitapūritaśāntapratyāhārottarādhārasamabhedena saptakumbhakeṣu madhye recitakumbhako 'yaṃ prathamābhyāse 'nekaniyamānapekṣatayā praśastaḥ} | \textit{sarvam etad agre prāṇāyāmaprakaraṇe sphuṭī bhaviṣyati} |}   

Die Beherrschung des Atems und die Beherrschung des Geistes gehören intrinsisch zusammen. Gleichzeitig hat \textit{prāṇāyāma} die Kraft, alle Sünden zu tilgen, was den Geist dazu befähigt sich zu konzentrieren und auf ein Ziel zu stabilisieren.\footnote{\citetitle{yogacandrika} 1.34 (Ed. p. 53): \textit{tad etābhyāṃ prāṇajaye cittajayas tayor avinābhāvāt prāṇāyāmasya sarvapāpanāśakatvāt pāpanivṛttyā ca cittam ekatra lakṣye sthiraṃ bhavati} |}

Abschließend legitimiert beglaubigt Nārāyaṇatīrtha die Verknüpfung von \textit{prāṇāyāma} und Haṭhayoga (\textit{prāṇāyāmasya haṭhayogatvam uktaṃ smṛtau}) mit dem berühmten Vers des \citetitle{yogabija} (148cd-149ab), in dem die Silbe ``\textit{ha}'' mit der Sonne und die Silbe ``\textit{ṭha}'' mit dem Mond verknüpft werden und \textit{haṭha} somit als die Vereinignug von Sonne und Mond begriffen wird.\footnote{\citetitle{yogacandrika} 1.34 (Ed. p. 53): \textit{hakāreṇa tu sūryo 'sau ṭhakāreṇendur ucyate} | \textit{sūryācandramasor aikyaṃ haṭha ity abhidhīyate ||}}

Der nächste Abschnitt der \textit{Yogasiddhāntacandrikā}, welcher Aspekte des Haṭhayoga diskutiert findet sich im Kontext des dritten Gliedes des \textit{aṣṭāṅgayoga}, welches beginnend mit \textit{sūtra} 1.34 beschrieben wird.  
...
Im Anschluss präsentiert Nārāyaṇatīrtha eine lange Liste von verschiedenen \textit{āsana}s, die im Detail beschrieben werden. Von insgesamt 84 \textit{āsana}s werden 38 detailliert beschrieben werden. \citeauthor{birch2018proliferation} beobachtene bereits 2018,\footnote{Cf. \citeauthor{birch2018proliferation} 2018, p. 105, fn. 9.} dass alle \textit{āsana}s früheren Yogatexten wie der \textit{Haṭhapradīpikā} (welche Nārāyanatı̄rtha als \textit{Yogapradīpa} nennt), der \citetitle{vasisthasamhita2017} und der \textit{Dharmaputrikā} entliehen wurden.\footnote{Eine Auflistung der 38 von 84, nachfolgend im Detail diskutierten \textit{āsana}s findet sich auf \citetitle{yogacandrika} 2.46 (Ed. p. 107-108): \textit{tac ca padma-siddha-bhadra-vīra-svastika-siṃha-daṇḍa-sopāśraya-paryaṅka-mayūra-kukkuṭa-uttānakukkuṭa-paścimatāna-matsyendrapīṭha-cakra-gomukha-karma-dhanu-mṛgasvastika-arddhacandra-añjalika-pīṭha-vajra-mukta-candra-arddhaprasāritaśava-kapāla-guruḍa-arddhāsana-kamala-krauñcaniṣadana-hastiniṣadana-uṣṭraniṣadanakapiniṣadana-yogāsana-yonyāsana-samasthāna-ādibhedena caturāśītiprakāram} | \textit{eteṣāṃ lakṣaṇāni yogapradīpādāv uktāni} | Die ausführlichen Beschreibungen der 38 \textit{āsana}s findet sich unmittelbar darauf folgend auf p. 108-114.}

In 2.47-48 spezifiziert Nārāyaṇatīrtha weitere Details zur Ausführung der Yogapositionen, die hier nicht weiter behandelt werden. Weitaus wichtiger zur Bestimmung von Nārāyaṇatīrtha's Haṭhayoga ist 2.49-51. Neben einer Ausführlichen Besprechung der drei Grundelemente von \textit{prāṇāyāma} - Ausatmung (\textit{recaka}, Einatmung (\textit{pūraka}) und Atemanhaltung (\textit{kumbhaka}) und deren korrekter Ausführung in 2.49-50, diskutiert Nārāyaṇatīrtha dann \textit{kevalakumbhaka}\footnote{Cf. \citetitle{yogacandrika} 1.34 (Ed. p. 116): `Yājñavalkya delcares its characteristic as follows - Having abandoned inhalation and exhalation, that comfortable restraint of breath is breath-control. This indeed is indeed taught as ``isolated retention''.' (\textit{asya ca lakṣaṇaṃ yājñavalkya āha- recakaṃ pūrakaṃ tyaktvā yat sukhaṃ vāyudhāraṇam} | \textit{prāṇāyāmo 'yam ity uktaḥ sa vai kevalakumbhakaḥ} ||}\footnote{See \citetitle{hathapradipika2024} 2.72-80 for the \textit{locus classicus} of all descriptions of \textit{kevalakumbhaka}.} als viertern Aspekt und ultimatives Resultat des \textit{prāṇāyāma}. Dieses \textit{kevalakumbhaka} wird in eine langwierigen Prozess mit immer subtiler werdendem Fortschritten durch Praxis des gewöhnlichen \textit{kumbhaka}, welches auch \textit{sahitakumbhaka} genant wird, erreicht. Erst wenn die körperlichen Kanäle durch die Praxis gereinigt sind und völliges Verlöschen der Bewegungen von Ausatmung und Einatmung einsetzt, entsteht \textit{kevalakumbhaka}.\footnote{Cf. \citetitle{yogacandrika} 2.51: \textit{evambhūta ubhayoḥ śvāsapraśvāsayor gativicchedaś caturthaḥ prāṇāyāma ity arthaḥ} | \textit{etena sahitakumbhakābhyāsa evāsyā 'sādhāraṇam} | \textit{yadā nāḍīviśuddhiḥ syād yoginastattvadarśinaḥ} | \textit{tadā vidhvastadoṣasya bhavet kevalasambhavaḥ} ||} Anschließend zitiert Nārāyaṇatīrtha sieben der acht \textit{kumbhaka}s\footnote{\citetitle{yogacandrika} 2.51, Ed. p. 118-121. Die sieben von Nārāyaṇatīrtha genannten \textit{kumbhaka}s sind: 1. \textit{sūryabhedana}; 2. \textit{ujjāyī}; 3. \textit{sītkā(ra)}; 4. \textit{śītalī}; 5. \textit{brahmarī}; 6.\textit{mūrchā}; und 7.\textit{bhastrikā}.} der \citetitle{hathapradipika2024} (ausgenommen \textit{plāvanī}, cf. \citetitle{hathapradipika2024} 2.71).\footnote{Cf. \citetitle{hathapradipika2024} 2.48-71.} Als nächstes werden die bereits im Kommentar zu 1.54 genannten anderen sieben \textit{kumbhaka}s erläutert. Der Kommentar zu 2.50 zitiert weiterhin diverse Text, wie \textit{Yogabhāskara}, \textit{Nandipurāṇa} und \textit{Mārkaṇḍeyapurāṇa} zum Thema \textit{prāṇāyāma}. Darüber hinaus werden die vier Stadien (\textit{avasthā}) der Praxis - \textit{ārambha}, \textit{ghāṭa}, \textit{paricaya} und \textit{niṣpatti}\footnote{See \citetitle{asiddhi} \textit{viveka} 19,21,29 and 31 for the oldest account of the four stages. Also cf. \citetitle{hathapradipika2024} 4.16–25.} - eingeführt.

Das Haṭhayoga des Nārāyaṇatīrtha besteht vornehmlich aus zwei der vier klassischen Hauptkategorien nach der \citetitle{hathapradipika2024}\footnote{Cf. \citetitle{hathapradipika2024} 1.56.} - \textit{āsana} und \textit{kumbhaka}. Auch die dritte Hauptkategorie - \textit{mudrā} - des Haṭhayoga nach der \citetitle{hathapradipika2024} findet sich in der \textit{Yogasiddhāntacandrikā}.Spannenderweise werden die zehn \textit{mudrā}s der \citetitle{hathapradipika2024}, genauso wie auch die \textit{ṣatkarma}s im Kontext von Karmayoga gelehrt und wird somit im folgenden Unterkapitel behandelt werden. Das vierte Hauptkategorie der \citetitle{hathapradipika2024}, \textit{nādānusandhāna}, findet sich nicht in der \textit{Yogasiddhāntacandrikā}.  
